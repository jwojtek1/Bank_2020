...
<br /><br />
<b>
Podaj numer konta na który chcesz wysłać przelew:</b>
...
{
//wyślij środki i podaj nr konta to dwie zmienne wklepane w formularzu. Są "POSTem", nrkonta i srodki to dane wczytane podczas logowania z bazy danych i dodane do sesji.
...
try 
{
	...
	{
		//jeśli kwota podana w formularzu będzie większa niż kwota jaką mamy na koncie, nie pozwoli nam wysłać przelewu
		if($wyslijsrodki > $srodki)
                    {
	                  $poprawnie=false;
	                  echo "Za mała ilość środków!";
                    }
					//jeśli nie jest tak jak wyżej, updatujemy tabelę użytkowników dodając do środków kwotę wklepaną w formularzu do podanego w formularzu numeru konta
					else
					{
						if($polaczenie->query("UPDATE `uzytkownicy` SET `srodki` = (`srodki`+'$wyslijsrodki') WHERE `uzytkownicy`.`nrkonta` = '$podajnrkonta';") )
							
							{
								echo "Przelew wysłany!";
							}
			...
			{
	                  $poprawnie=false;
	                  echo " Przelew nie został wysłany!";
            }
			// Tutaj kolejne zapytanie do bazy, tym razem jednak kwota podana w formularzu ma zostać odjęta od naszego numeru konta.
			else
			{
